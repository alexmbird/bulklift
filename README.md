...
| `config.transcoding.rewrite_metadata` | - | `{'track': null, 'artist':'{artist}'}` | Rewrite selected tags in the target files.  Value can be either `null` (meaning delete the field) or a format-string which will have metadata from the Bulklift manifest interpolated into place. Valid metadata field names are listed [here](https://wiki.multimedia.cx/index.php?title=FFmpeg_Metadata#MP3). |
